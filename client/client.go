...
type Client struct {
	AccessKey  string
	Endpoint   string
	APIVersion string // Defaults to 2021-10-01-preview
}
...
const sendEndpoint = "/emails:send"
const statusEndpoint = "/emails/%s/status"
...
func (c *Client) GetStatus(messageID string) (status string, err error) {
	req, err := http.NewRequest("GET", c.Endpoint+fmt.Sprintf(statusEndpoint, messageID)+"?api-version="+c.APIVersion, nil)
	if err != nil {
		return "", err
	}
